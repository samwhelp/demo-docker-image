...
podman run -it --replace --privileged -v "${REF_PORT_DIR_PATH}:/port" -v "${REF_PLAN_DIR_PATH}:/opt/prj" -w "/opt/prj" --name "debian-iso-factory-25.10-run" "${REF_MASTER_BOX_DISTRO_NAME}" "/opt/prj/src/build.sh"

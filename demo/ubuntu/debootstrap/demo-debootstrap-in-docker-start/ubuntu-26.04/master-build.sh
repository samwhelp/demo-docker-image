...
podman run -it --replace --privileged -v "${REF_PORT_DIR_PATH}:/port" -v "${REF_PLAN_DIR_PATH}/src:/opt/prj/src" -w "/opt/prj" --name "ubuntu-iso-factory-26.04-run" "${REF_MASTER_BOX_DISTRO_NAME}" "/opt/prj/src/build.sh"
